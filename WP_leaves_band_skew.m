function [meso_coeff, therm_coeff] = WP_leaves_band_skew(meso,therm, my_lengths)
%% Wavelet Packet analysis, power series approximation 
% This function looks at the scale determined by the shorter of the two
% proteins, and goes two to four levels above that. It retains the
% frequency bands at each leaf determined by scale, and returns the skew of
% each leaf. 
...
for j = 1:abs(length(meso))
    ...
    for k = 1:abs(length(J_meso))
        ...
        coeff_meso(1,k) = skewness(wpcoef(WPT_meso,[level-2,J_meso(k)])); 
        
        coeff_therm(1,k) = skewness(wpcoef(WPT_therm,[level-2, J_therm(k)])); 
